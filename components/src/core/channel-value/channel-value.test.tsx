...
describe('ChannelValue', () => {
    ...
    describe('units Text element', () => {
        it('only renders one Text element when units are not present', () => {
            const instance = TestRenderer.create(<ChannelValue value={123} />).root;

            expect(getChildTextComponents(instance)).toHaveLength(1);
        });

        it('renders units before value when prefix == true', () => {
            const instance = TestRenderer.create(<ChannelValue value={123} prefix={true} units="hz" />).root;
        ...
        it('renders units after value when prefix == false', () => {
            const instance = TestRenderer.create(<ChannelValue value={123} prefix={false} units="hz" />).root;
        ...
        it('renders units after value by default', () => {
            const instance = TestRenderer.create(<ChannelValue value={123} units="hz" />).root;
